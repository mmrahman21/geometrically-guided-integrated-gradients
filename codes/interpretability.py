...
def get_multiple_direc_deriv_saliency(model, loaderSal, no_of_directions, iterations, lr, device):
    
    '''
    Compute multiple directional vector based saliency - Algo 1
    '''
#     model.eval()
...
#     model.train()
    ...
    for i, data in enumerate(loaderSal):
        ...
        for n in range(no_of_directions):
            x_random = torch.normal(0, 1.0, size=x.shape).to(device)
            r = x_random - x
            r = r / r.norm()
            ...
            for j in range(iterations):
                ...
                with torch.backends.cudnn.flags(enabled=False): 
                    out = model(xn)
                    (func_out, jac_vec_prod) = jvp(model, xn, r)
                    if jac_vec_prod[0, preds] < max_grad:
                        print(f"Sub {i}:Stopping at {j}")
                        break
                    else:
                        max_derivative_step = j
                        max_grad = jac_vec_prod[0, preds]
            ...
            x_p = x + max_derivative_step * lr * r
            out_p = model(x_p)
            grad_outputs = torch.zeros(out.shape).to(device)
            grad_outputs[:, preds] = 1
            
            gradients = grad(outputs=out_p, inputs=x_p, grad_outputs=grad_outputs, retain_graph=False)[0]
            saliencies.append(np.squeeze(gradients.cpu().detach().numpy()))  # saving grad for checking instead of accumulation
...
def get_multiple_direc_deriv_saliency2(model, loaderSal, no_of_directions, iterations, lr, device):
    
    '''
    Compute multiple directional vector based saliency - Algo 2
    '''
...
#     model.train()
    ...
    alphas = torch.linspace(start=0.0, end=1.0, steps=iterations+1).to(device)
    ...
    for i, data in enumerate(loaderSal):
        ...
        for n in range(no_of_directions):
            ...
            base = x + iterations * lr * r
            
            interp_images = construct_interpolate_images(baseline = base,
                                   image=x,
                                   alphas=alphas)
            
            v = torch.cat((iterations+1)*[r])
            
            model.zero_grad()
            (func_out, jac_vec_prod) = jvp(model, interp_images, v)
            direct_derivatives = jac_vec_prod[:, preds]
            direct_derivatives = direct_derivatives.squeeze(1)
            
            direct_derivatives = torch.as_tensor(direct_derivatives)
            direct_derivatives = (direct_derivatives - torch.min(direct_derivatives)) / (torch.max(direct_derivatives) -torch.min(direct_derivatives))
            direct_derivatives = torch.nan_to_num(direct_derivatives)

            rev_derivatives = direct_derivatives.cpu().numpy()[::-1]
    
            max_grad = rev_derivatives[0]
            max_steps = len(direct_derivatives)
            max_derivative_idx = max_steps - 1
            rev_order = 0

            for j in range(1, max_steps):
                if rev_derivatives[j] < max_grad:
                    print(f"Sub {i}:Stopping at {j}")
                    break
                else:
                   
                    max_derivative_idx = max_steps - 1 - j 
                    rev_order = j
                    max_grad = rev_derivatives[j]
    
            x_p = interp_images[max_derivative_idx]
            x_p = x_p.unsqueeze(0)
            out_p = model(x_p)
            grad_outputs = torch.zeros(out.shape).to(device)
            grad_outputs[:, preds] = 1
...
def get_direct_vec_saliency(model, loaderSal, iterations, lr, device):
    
    '''
    Compute Directional vector based saliency
    '''
...
#     model.train()
    ...
    for i, data in enumerate(loaderSal):
        ...
        x_random = torch.normal(0, 1.0, size=x.shape).to(device)
        direct_vec = x_random - x
        direct_vec = direct_vec / direct_vec.norm()
...
#             torch.backends.cudnn.enabled=False
            ...
            with torch.backends.cudnn.flags(enabled=False):
                ...
                (func_out, jac_vec_prod) = jvp(model, x.data, direct_vec)
        ...
        saliencies = np.stack(saliencies, axis=0)
        print(saliencies.shape)
        all_saliencies[i] = np.mean(saliencies, axis=0)
...
def get_extended_vec_saliency(model, loaderSal, baseline, iterations, lr, device):
    
    '''
    Compute Extended vector based saliency
    '''
...
#     model.train()
    ...
    for i, data in enumerate(loaderSal):
        ...
        main_out = model(x)
        main_prob_scores = F.softmax(main_out, dim=1)
     
        _, preds = torch.max(main_out.data, 1)
        
        r = x / x.norm()
        r = r.to(device)
        image = copy.deepcopy(x).to(device)
        ...
        for j in range(iterations):
            ...
            path_prob_scores = F.softmax(out, dim=1)

            grad_outputs = torch.zeros(out.shape).to(device)
            grad_outputs[:, preds] = 1
            
            if i % 50 == 0:
                print(f'Iter: {j} Main Logit Score: {main_out[:, y]}, Main Prob Score: {main_prob_scores[:, y]}')
                print(f'Iter: {j} Path Logit Score: {out[:, y]}, Path Prob Score: {path_prob_scores[:, y]}')
                
            gradients = grad(outputs=out, inputs=image, grad_outputs=grad_outputs, retain_graph=False)[0]
            
            if j <= iterations // 2:
                image.data = image.data - lr * r
                
            else:
                image.data = image.data + lr * r
                
            if j == iterations // 2:
                image.data = copy.deepcopy(x).to(device)
